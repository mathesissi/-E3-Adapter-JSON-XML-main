import { XMLService } from "../services/LegacyXMLSystem";
import { Usuario } from "../models/Usuario";

export class JsonToXmlAdapter implements XMLService {
    constructor(private legacySystem: XMLService) {}
    
    processarCadastro(jsonData: string): Usuario {
        try {
            ...
            throw new Error(`Adaptação falhou: ${(error as Error).message}`);
        }
    }

    private parseAndValidate(jsonData: string): any {
        try {
